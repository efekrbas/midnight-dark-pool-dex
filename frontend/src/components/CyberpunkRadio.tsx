...
export default function CyberpunkRadio() {
  ...
  return (
    <div className="fixed bottom-28 right-6 z-40 bg-zinc-900/90 backdrop-blur-2xl border border-zinc-700/40 rounded-2xl shadow-2xl p-3 w-64 animate-fadeIn">
      ...
      <div className="flex items-center justify-between mb-2">
        ...
        <button onClick={() => setIsVisible(false)} className="text-zinc-500 hover:text-white p-0.5">
          <X className="w-3 h-3" />
        </button>
      </div>
      ...
      <div className="flex items-end gap-[2px] h-6 mb-2 px-1">
        {bars.map((h, i) => (
          <div
            ...
            style={{
              height: `${h}px`,
              background: `linear-gradient(to top, rgba(161,161,170,0.8), rgba(113,113,122,0.6))`,
      ...
      <div className="flex items-center justify-between">
        <div className="min-w-0 flex-1 mr-2">
          ...
          <p className="text-[10px] text-zinc-400 font-mono truncate">{track.artist}</p>
        </div>
        <div className="flex items-center gap-1.5">
          <button
            onClick={togglePlay}
            className="w-8 h-8 rounded-full bg-zinc-600 hover:bg-zinc-500 flex items-center justify-center text-white transition-all shadow-md"
          >
            {isPlaying ? <Pause className="w-3.5 h-3.5" /> : <Play className="w-3.5 h-3.5 ml-0.5" />}
          </button>
          <button
            onClick={nextTrack}
            className="w-7 h-7 rounded-full bg-zinc-800 hover:bg-zinc-700 flex items-center justify-center text-zinc-400 hover:text-white transition-all"
          >
