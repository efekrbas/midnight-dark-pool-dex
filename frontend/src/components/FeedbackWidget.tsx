  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setStatus("idle");
        notify("Submission Failed", errData.error || "Failed to submit feedback. Server returned an error.", "error");
      }
    } catch (err: unknown) {
      const error = err as Error;
      console.error(error);
      setStatus("idle");
      notify("Network Error", error.message || "Could not connect to the feedback server.", "error");
    }
  };
  ...
  return (
    <>
      <button
        ...
        className="fixed bottom-6 right-6 p-4 rounded-full bg-teal-600 text-white shadow-lg shadow-teal-500/20 hover:scale-105 transition-transform z-50 flex items-center justify-center group"
      >
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="flex gap-2">
                      {(["Bug Report", "Feature Request", "General"] as const).map(t => (
                        <button
                          ...
                          type="button"
                          onClick={() => setType(t)}
                          className={`text-xs px-2.5 py-1.5 rounded-full border transition-colors ${type === t
                  ...
                  <div>
                    <textarea
                      ...
                      className="w-full h-24 bg-white/5 border border-white/10 rounded-lg p-3 text-sm focus:outline-none focus:border-teal-500/50 resize-none text-white"
                      value={text}
